...
def admin_view():
    if is_admin():
        return redirect(url_for('admin.admin_graphs', _external=True))
...
def admin_pages(route):
    ...
    if route and request.method == 'POST':
        ...
        if page:
            ...
            db.session.commit()
            db.session.close()
            return redirect(url_for('admin.admin_pages', _external=True))
...
def new_container():
    ...
    if set(name) <= set('abcdefghijklmnopqrstuvwxyz0123456789-_'):
        return redirect(url_for('admin.list_container', _external=True))
...
def admin_files(chalid):
    ...
    if request.method == 'POST':
        if request.form['method'] == "delete":
            ...
            db.session.commit()
            db.session.close()
            return redirect(url_for('admin.admin_chals', _external=True))
...
def ban(teamid):
    ...
    db.session.commit()
    db.session.close()
    return redirect(url_for('admin.admin_scoreboard', _external=True))
